...
int isin(vEB *tree, int x) {
    if (tree->min == -1) return 0;
    if (x == tree->min || x == tree->max) return 1;
    ...
    // same min 
    if (tree->cluster[high] == NULL) return 0;
...
void delete(vEB *tree, int x) {
    if (tree->min == -1 || !isin(tree, x)) return;
